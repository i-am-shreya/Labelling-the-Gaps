# -*- coding: utf-8 -*-
"""
Created on Thu Jan  9 18:10:35 2020
...
#    
#    model = Model(input=[inputs_label,inputs_unlabel,inputs_label_dash], output=[x_embedding, output_label])
#    model.summary()
#
...
#    attention_mul = Dense(64)(attention_mul)
    ...
    output_label = Dense(2, activation='sigmoid',name = 'Output_label',kernel_regularizer=regularizers.l2(0.01),
...
# data loading
# =============================================================================
with h5py.File('data_triplet_vgg-fc6_cave.h5', 'r') as hf:
...
model.save('./models/video_eyegaze_onelabel_kl_sgd0.01_normalizedlabel_cave_1000.h5')
...
model.load_weights('./models/video_eyegaze_onelabel_kl_sgd0.01_normalizedlabel_cave_1000.h5')
...
Y_dash = np.zeros([len(original_unlabel),2])
Y_dash[:,0] = (y_pred[:,0]*(np.ones(len(original_unlabel[:,0]))*
      max(original_unlabel[:,0])-np.ones(len(original_unlabel[:,0]))*
      min(original_unlabel[:,0])))+np.ones(len(original_unlabel[:,0]))*min(original_unlabel[:,0]) 
Y_dash[:,1] = (y_pred[:,1]*(np.ones(len(original_unlabel[:,1]))*
      max(original_unlabel[:,1])-np.ones(len(original_unlabel[:,1]))*
      min(original_unlabel[:,1])))+np.ones(len(original_unlabel[:,1]))*min(original_unlabel[:,1])
#Y_dash[:,2] = (y_pred[:,2]*(np.ones(len(original_unlabel[:,2]))*
#      max(original_unlabel[:,2])-np.ones(len(original_unlabel[:,2]))*
#      min(original_unlabel[:,2])))+np.ones(len(original_unlabel[:,2]))*min(original_unlabel[:,2]) 

print(mean_absolute_error(original_unlabel, Y_dash))
print(mean_absolute_error(original_unlabel[:,0], Y_dash[:,0]))
print(mean_absolute_error(original_unlabel[:,1], Y_dash[:,1]))